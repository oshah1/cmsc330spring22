...
let rec parse_expr toks =
  ...
  if t <> Tok_DoubleSemi then
    ...
    and parse_exp toks =
      ...
      | Some Tok_Let -> 
                    let (tok3, m) = parse_let toks in
                    (tok3, Let m)

      | Some Tok_Fun -> let tok2 = match_token toks Tok_Fun in
                    let (tok3, m) = parse_fun tok2 in
                    (tok3, FunctionCall m)
      | Some Tok_If -> 
                  let (t2, m) = parse_if toks in
                  (t2, m)              
      ...
      and parse_let toks =
        match lookahead toks with
        | Some Tok_Let -> let tok2 = match_token toks Tok_Let in
                          let (recurse,tok3) = match lookahead tok2 with
                                      | Some Tok_Rec -> (true,match_token tok2 Tok_Rec)
                                      | _ -> (false,tok2) in
                                     begin match (lookahead tok3), (lookahead_many tok3 1) with
                                      | Some Tok_ID i, Some Tok_Equal -> match_many tok3 [Tok_ID i;Tok_Equal] in
                                                                        let (tok4, exp) = parse_exp tok3 in
                                                                        let (tok5, exp2) = parse_exp (match_token tok4 Tok_In) in
                                                                        (tok5, Let (i,recurse,exp1,exp2))
                                      | _,_ ->  raise (InvalidInputException "Let") end
        | _ -> raise (InvalidInputException "Let")
      ...
      and parse_and toks = 
      let let (tok1, exp1) = parse_equal toks in
    ...
    let (tok2, exp2) = parse_second tok1 in
    (tok2, FunctionCall (exp1, exp2))

    and parse_second toks =
    match lookahead toks with
    | Some Tok_Int i -> let tok2 = match_token (Tok_Int i) in
                  (tok2, Value (Int i))
                  | Tok_Bool b -> let tok2 = match_token toks (Tok_Bool b) in
                  (tok2, Value (Bool b))
    ...
    match lookahead toks with
    | Tok_Int i -> let tok2 = match_token toks (Tok_Int i) in
                  (tok2,Value (Int i))

    | Tok_Bool b -> let tok2 = match_token toks (Tok_Bool b) in
                  (tok2, Value (Bool b))
    | Tok_String st -> let tok2 = match_token toks (Tok_String st) in
                    (tok2, Value (String st))
    | Tok_ID id -> let tok2 = match_token toks (Tok_ID s) in
                    (tok2, ID id)
    | Tok_LParen -> let tok2 = match_token toks (Tok_LParen) in
                    let (tok3, e) = parse_exp tok2 in (*parse the
                                                        expression*)
                    let tok4 = match_token tok3 (Tok_RParen) in
                    (tok4,e)
